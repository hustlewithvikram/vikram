...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <div
      className="w-full bg-[#111] dark:bg-neutral-950 font-sans md:px-10"
      ref={containerRef}
    >
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-32 self-start max-w-xs lg:max-w-sm md:w-full">
              ...
              {/* <div className="h-10 absolute left-3 md:left-3 w-10 rounded-full bg-white dark:bg-black flex items-center justify-center">
                <div className="h-4 w-4 rounded-full bg-black dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 p-2" />
              </div> */}
              <h3 className="hidden z-[2] md:block text-xl md:pl-20 md:text-5xl font-roboto font-extrabold text-black dark:text-neutral-500">
                {item.title}
              </h3>
            </div>

            <div className="relative pl-20 pt-12 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-2xl mb-4 text-left font-semibold text-neutral-950 dark:text-neutral-500">
                {item.title}
              </h3>
              {item.content}
              {""}
